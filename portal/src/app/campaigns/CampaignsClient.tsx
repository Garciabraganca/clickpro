    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="border-b border-slate-800">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-6 py-6">
          <div>
            <h1 className="text-2xl font-semibold" title="Área para criar campanhas e enviar mensagens em massa">
              Painel de Campanhas
            </h1>
            <p className="text-sm text-slate-400" title="Gerencie todas as suas campanhas de WhatsApp aqui">
              Crie campanhas e monitore progresso de envios.
            </p>
          </div>
          <ApiConfigCard
            baseUrl={baseUrl}
      ...
      <main className="mx-auto grid max-w-6xl grid-cols-1 gap-6 px-6 py-8 lg:grid-cols-[1.2fr_1fr]">
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 lg:col-span-2">
          ...
          <p className="mb-4 text-sm text-slate-400" title="Verifique os requisitos antes de criar uma campanha">
            Checklist para criar campanha:
          </p>
          <ul className="space-y-2 text-sm">
            <li className="flex items-center gap-2" title="Você precisa criar e aprovar um template antes de usá-lo">
              ...
              <span className={approvedTemplates.length > 0 ? "text-slate-300" : "text-slate-400"}>
                ...
                {approvedTemplates.length === 0 && (
                  <span className="ml-2 text-xs text-slate-500">(vá para Templates e crie um)</span>
                )}
              </span>
            </li>
            <li className="flex items-center gap-2" title="Você deve importar ao menos um contato">
              ...
              <span className={contacts.length > 0 ? "text-slate-300" : "text-slate-400"}>
                Contatos importados
                {contacts.length === 0 && (
                  <span className="ml-2 text-xs text-slate-500">(vá para Contatos e importe um CSV)</span>
                )}
              </span>
            </li>
          </ul>
          {approvedTemplates.length > 0 && contacts.length > 0 && (
            <div
              ...
              title="Você está pronto para criar campanhas"
            >
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
          <h2 className="text-lg font-semibold" title="Formulário para criar uma nova campanha">
            Nova Campanha
          </h2>
          <div className="mt-4 space-y-4">
            <div>
              <label className="text-xs text-slate-400" title="Nome para identificar sua campanha">
                Nome
              </label>
              <input
                value={name}
            ...
            <div>
              <label className="text-xs text-slate-400" title="Selecione um template aprovado para enviar">
                Template aprovado
              </label>
              <select
                value={templateId}
              ...
              {approvedTemplates.length === 0 && (
                <p className="mt-1 text-xs text-amber-400" title="Você precisa de um template aprovado">
                  ⚠️ Nenhum template aprovado disponível
                </p>
              )}
            </div>
            <div>
              <label className="text-xs text-slate-400" title="Quantidade máxima de mensagens enviadas por minuto">
                Rate limit (msg/min)
              </label>
              <input
                value={rateLimit}
                ...
                placeholder="20"
              />
              <p className="mt-1 text-xs text-slate-500" title="Dica de rate limit">
                Recomendado: 20-30 msg/min para evitar bloqueios
              </p>
            </div>
            <div>
              <label className="text-xs text-slate-400" title="Escolha quais contatos receberão esta campanha">
                Selecionar contatos
              </label>
              <input
                value={search}
              ...
              <div className="mt-3 max-h-56 space-y-2 overflow-y-auto rounded-xl border border-slate-800 bg-slate-950 p-3">
                ...
                {contacts.map((contact) => (
                  <label
                    key={contact.id}
                    className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 text-sm text-slate-200 hover:bg-slate-900"
                    title={`Selecionar ${contact.name || contact.phone}`}
                  >
                    <input
                      type="checkbox"
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
          <h2 className="text-lg font-semibold" title="Histórico de todas as suas campanhas">
            Histórico
          </h2>
          <p className="mt-1 text-xs text-slate-500" title="Informação sobre as campanhas">
            Acompanhe o progresso de envio das suas campanhas
          </p>
          <div className="mt-4 space-y-3">
            {campaigns.length === 0 && (
              <p className="text-sm text-slate-400" title="Nenhuma campanha criada ainda">
                Nenhuma campanha criada. Crie sua primeira campanha ao lado.
              </p>
            )}
            {campaigns.map((campaign) => (
              <div key={campaign.id} className="rounded-xl border border-slate-800 bg-slate-950 p-4" title="Detalhes da campanha">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-white" title="Nome da campanha">
                      {campaign.name}
                    </p>
                  ...
                  <span
                    ...
                    title={`${campaign.sent_contacts} de ${campaign.total_contacts} mensagens enviadas`}
                  >
                ...
                <div
                  ...
                  title={`Progresso: ${Math.round((campaign.sent_contacts / campaign.total_contacts) * 100)}%`}
                >
                  <div
